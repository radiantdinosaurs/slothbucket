...
describe('User Controller', () => {
    describe('createUser', () => {
        it('rejects with an error if Mongoose finds a duplicate users', (done) => {
            ...
            controller.createUser(mockUserData).then(() => {
                done(new Error('Creating users should not have been successful'))
            }).catch((error) => {
                expect(error).to.be.an.instanceOf(Error).which.has.property('message', 'duplicate users found')
                done()
            })
        })
        ...
        it('resolves if Mongoose successfully creates the users in the database', (done) => {
            ...
            controller.createUser(mockUserData).then((output) => {
