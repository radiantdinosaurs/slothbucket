...
const mockReturnError = {
    incompleteArguments: function() {
        return new Error('incomplete arguments')
    },
...
describe('Parse Tensorflow', () => {
    describe('parseTensorFlowResult', () => {
        it('checks that argument (result) is present', () => {
            expect(() => {
                parseTensorFlow.parseTensorFlowResult(undefined)
            })
                .to.throw(Error)
                .with.property('message', 'incomplete arguments')
        })
        it('checks that param is a string', () => {
            expect(() => {
                parseTensorFlow.parseTensorFlowResult(2)
            })
                .to.throw(Error)
                .with.property('message', 'incomplete arguments')
        })
        it('can tolerate malformed strings', () => {
            const result = parseTensorFlow.parseTensorFlowResult(
                'malformed-strings,whoa'
            )
