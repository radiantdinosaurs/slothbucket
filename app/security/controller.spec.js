...
describe('Security Controller', () => {
    describe('handleAuthentication', () => {
        ...
        it('sends a response if finding the users is not successful', done => {
            ...
            mockUserController.findUserByUserName = () =>
                new Promise((resolve, reject) => reject(new Error('not found')))
            controller.handleAuthentication[1](request, response, next)
            done()
        })
        it('sends a response if verifying the users is not successful', done => {
            validationObject.isEmpty = () => true
            mockUserController.findUserByUserName = () =>
                new Promise(resolve => resolve({ user: 'user' }))
            mockVerify.verifyPassword = () =>
                new Promise((resolve, reject) =>
                    reject(new Error('not verified'))
                )
            controller.handleAuthentication[1](
                request,
                validatingResponse({ status: undefined, error: 'not verified' })
            )
            done()
        })
